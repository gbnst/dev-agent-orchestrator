...
- Labels for metadata: devagent.managed, devagent.project_path, devagent.template, devagent.remote_user, devagent.sidecar_type
...
- Sidecar architecture: Proxy sidecars use compose project name as ParentRef (from com.docker.compose.project label); grouped with app containers by compose project
...
- Sidecar ParentRef is compose project name (from com.docker.compose.project label)
- Proxy health check waits for container to be running (30s timeout)
- Container and network names are auto-generated by compose (no hardcoded container_name); proxy addressed via service name "proxy" in compose network
